...
public partial class SystemManage_UserLoginTotal : BasePage
{
    ...
    private void GetTotalData()
    {
        ...
        { 
            ...
            Bind(SessionBox.GetUserSession().DeptNumber, cboDept.Value.ToString().Trim(), txtName.Text.Trim(), txtUser.Text.Trim());

        }
    }
    private void Bind(string maindept,string deptnm, string psn)
    {
        DataSet ds = GetKaoHeInfo.GetUserLoginTotal(dateBegin.Date, dateEnd.Date, maindept,deptnm, psn);
    ...
    {
        DataSet ds = GetUserLoginTotal(dateBegin.Date, dateEnd.Date, maindept, deptnm, psn,username);
        gvUserOptTotal.DataSource = ds;
        gvUserOptTotal.DataBind();
    }
    ...
    private DataSet GetUserLoginTotal(DateTime dateBegin, DateTime dateEnd, string maindept, string deptnm, string psn, string username)
    {
        //string strSql = string.Format("select r.username,r.personnumber,r.name,r.deptnumber,r.deptname,r.maindeptid,r.maindept,nvl(w.LoginCount,0) LoginCount from (select distinct u.userid,u.username,u.personnumber,p.name,kq.deptnumber deptnumber,kq.deptname deptname,dept.deptnumber maindeptid,dept.deptname maindept from sf_user u join person p on u.personnumber = p.personnumber left join department kq on p.areadeptid=kq.deptnumber left join department dept on p.maindeptid=dept.deptnumber) r left join ( SELECT username,count(DISTINCT case when to_number(to_char(vuserlog.activetime,'hh24')) between 0 and 12 then TO_CHAR(vuserlog.activetime,'yyyy-mm-dd')||'上午' when to_number(to_char(vuserlog.activetime,'hh24')) between 12 and 24 then TO_CHAR(vuserlog.activetime,'yyyy-mm-dd')||'下午' end) LoginCount FROM vuserlog where ActiveTime between to_date('{0}','YYYY-MM-DD') and to_date('{1}','YYYY-MM-DD') and activetype='登录' group by  vuserlog.username) w on r.username = w.username where r.username !='yu'", dateBegin.ToShortDateString(), dateEnd.ToShortDateString());
        string strSql = string.Format("select r.username,r.personnumber,r.name,r.deptnumber,r.deptname,r.maindeptid,r.maindept,nvl(w.LoginCount,0) LoginCount from (select distinct u.userid,u.username,u.personnumber,p.name,kq.deptnumber deptnumber,kq.deptname deptname,dept.deptnumber maindeptid,dept.deptname maindept from sf_user u join person p on u.personnumber = p.personnumber left join department kq on p.areadeptid=kq.deptnumber left join department dept on p.maindeptid=dept.deptnumber) r left join ( SELECT username,count(DISTINCT case when to_number(to_char(vuserlog.activetime,'hh24')) between 0 and 12 then TO_CHAR(vuserlog.activetime,'yyyy-mm-dd')||'上午' when to_number(to_char(vuserlog.activetime,'hh24')) between 12 and 24 then TO_CHAR(vuserlog.activetime,'yyyy-mm-dd')||'下午' end) LoginCount FROM vuserlog where ActiveTime between to_date('{0}','YYYY-MM-DD') and to_date('{1}','YYYY-MM-DD')+1 and activetype='登录' group by  vuserlog.username) w on r.username = w.username where r.username !='yu'", dateBegin.ToShortDateString(), dateEnd.ToShortDateString());
        if (maindept != "-1")
        {
